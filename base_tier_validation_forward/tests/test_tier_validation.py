...
class TierTierValidation(CommonTierValidation):
    def test_01_forward_tier(self):
        ...
        wizard = (
            ...
            .create(
                {
                    "forward_reviewer_id": self.test_user_1.id,
                    "forward_description": "Please review again",
                }
            )
        )
        wizard.add_forward()
        ...
        record.invalidate_recordset()
        self.assertTrue(
            record.review_ids.filtered(lambda review: not review.definition_id)
        )
        # User 1 validate
        ...
        wizard = (
            ...
            .create(
                {
                    "comment": "Forward tier is reviewed",
                }
            )
        )
        wizard.add_comment()
